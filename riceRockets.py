...
class Sprite:
    ...
    def collide(self, other_object):
        collision = False
        if dist(self.get_pos(), other_object.get_pos()) < (self.get_r() + other_object.get_r()):
            collision = True
        return collision    
...
def rock_spawner():
    ...
    if len(rock_group) < 10 and started:
        if (dist(a_rock.get_pos(), my_ship.get_pos())) >(a_rock.get_r() + (1.5 * my_ship.get_r()) + 100):
            rock_group.add(a_rock)
    # This is used to stop the soundtrack when you close the window
    # Remove it if you are using Firefox or it will throw an exeption
